...
const App: React.FC = () => {
  ...
  }, [contentType, gameMode, items]); // removed 'view' dependency to prevent aggressive resets
  ...
  const handleLockInteraction = () => {
    ...
    if (lockTapCount.current === 1) speak("Toque três vezes para desbloquear");
  ...
  const getTitle = () => {
    ...
    if (gameMode === GameMode.MEMORY) return "Jogo da Memória"; // Handled by component mostly
              ...
              <div className="bg-white rounded-[2rem] shadow-2xl p-8 md:p-12 w-full max-w-md animate-in zoom-in">
                  ...
                  <form onSubmit={handleCompleteProfile} className="space-y-4">
                    <div className="space-y-1">
                        ...
                        <input type="text" required placeholder="Ex: Ana" className="w-full px-5 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:border-blue-400 font-bold text-slate-600" value={profileCompletionData.name} onChange={e => setProfileCompletionData(p => ({...p, name: e.target.value}))} />
                    </div>
                    <div className="space-y-1">
                        <label className="text-xs font-bold text-slate-500 ml-3 uppercase">Idade</label>
                        <input type="number" required placeholder="Anos" className="w-full px-5 py-3 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:outline-none focus:border-blue-400 font-bold text-slate-600" value={profileCompletionData.age} onChange={e => setProfileCompletionData(p => ({...p, age: e.target.value}))} />
                    </div>
                    <button type="submit" className="w-full py-4 bg-green-500 text-white rounded-2xl font-black text-lg shadow-lg shadow-green-200 hover:scale-[1.02] active:scale-95 transition-all mt-4 flex items-center justify-center gap-2"><CheckCircle2 /> Concluir</button>
                    <button type="button" onClick={handleLogout} className="w-full py-2 text-slate-400 font-bold text-sm hover:text-slate-600">Cancelar e Sair</button>
                  </form>
              </div>
          </div>
      );
        ...
        <div className="flex-1 overflow-hidden relative p-2 md:p-4 flex flex-col items-center justify-center min-h-0">
            ...
            {view === 'GAME' && gameMode === GameMode.QUIZ && quiz.target && (
                <div className="w-full text-center py-2 md:hidden animate-in fade-in slide-in-from-top-2 shrink-0">
                     <span className="text-xl font-black text-slate-700 drop-shadow-sm">{getTitle()}</span>
                </div>
            )}
                ...
                <div className="w-full max-w-md bg-white rounded-3xl shadow-lg border border-slate-100 p-6 md:p-8 animate-in zoom-in-95 duration-200">
                    ...
                    <div className="space-y-6">
                        <div>
                            ...
                            <select className="w-full p-4 rounded-xl border-2 border-slate-100 bg-slate-50 text-base font-semibold text-slate-600 focus:outline-none focus:border-blue-300 transition-colors cursor-pointer" onChange={(e) => setSelectedVoice(voices[parseInt(e.target.value)])} value={voices.indexOf(selectedVoice as SpeechSynthesisVoice)}>
                        ...
                        <div className="pt-6 border-t border-slate-100">
                            <button onClick={handleLogout} className="w-full flex items-center justify-center gap-3 px-6 py-4 rounded-2xl text-base font-black transition-all bg-red-50 text-red-500 hover:bg-red-100 hover:text-red-600 border border-red-100"><LogOut size={20} /><span>Sair da Conta</span></button>
                        </div>
                    </div>
                </div>
            )}
            ...
            {view === 'GAME' && (
                <>
                    {quiz.feedback && (<div className="absolute inset-0 z-50 flex items-center justify-center bg-white/20 backdrop-blur-sm animate-in fade-in duration-200"><div className="text-9xl animate-bounce filter drop-shadow-2xl">{quiz.feedback === 'correct' ? '🌟' : '🤔'}</div></div>)}
